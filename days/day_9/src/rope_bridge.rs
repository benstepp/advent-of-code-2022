...
impl RopeBridge {
    ...
    pub fn unique_tail_locations(&self) -> i32 {
        ...
        for step in &self.steps {
            for _ in 0..step.count {
                move_head(&mut head_x, &mut head_y, &step.direction);
                maybe_move_tail(&mut head_x, &mut head_y, &mut tail_x, &mut tail_y);
...
fn move_head(head_x: &mut i32, head_y: &mut i32, direction: &Direction) {
    match direction {
...
fn maybe_move_tail(head_x: &mut i32, head_y: &mut i32, tail_x: &mut i32, tail_y: &mut i32) {
...
fn move_tail(head_x: &mut i32, head_y: &mut i32, tail_x: &mut i32, tail_y: &mut i32) {
    if head_x == tail_x {
...
fn move_coords(head: &mut i32, tail: &mut i32) {
    match head.cmp(&tail) {
